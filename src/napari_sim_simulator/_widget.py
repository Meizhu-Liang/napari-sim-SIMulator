...
class SIMulator():
    ...
    def parameters(self, SIM_mode=Sim_mode.HEXSIM_RIGHT_ANGLES, Polarisation=Pol.IN_PLANE, Acceleration=list(Accel)[-1],
                   ...
                   # wavelength:float=0.52, npoints:int=500, zrange:float=7.0, dz:float=0.4, fwhmz:float=3.0, show_psf=False, show_otf=False):
...
# @magicgui
# def raw_images_SIMulator():


if __name__ == '__main__':

    s = SIMulator(napari.Viewer())
    s.run()
    s.viewer.window.add_dock_widget(s.w,
                                       name='my second app',
